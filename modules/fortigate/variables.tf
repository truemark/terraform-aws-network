...
variable "subnets" {
  description = <<-EOF
        ...
        The format of subnet name should be follow the name prefix of \"fgt_login_\", \"fgt_internal_\", \"tgw_attachment_\", \"gwlbe_\", "ngw_", \"privatelink_\". Specify the target subnet you needed.
        Format:
        ```
            subnets = {
                \<Subnet name\> = {
...
variable "spk_vpc" {
  description = <<-EOF
    Spoke VPC configuration. VPC ID and Subnet IDs shold be provided. This module will create Transit Gateway Attachments under each subnet, and create VPC route tables with the subnet associated.
    Format:
    ```
        spk_vpc = {
            \<Spoke VPC name\> = {
